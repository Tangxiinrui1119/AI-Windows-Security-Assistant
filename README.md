...
项目以 **Python** 作为主控、数据处理与 AI 分析层：把 Windows 的系统信息整理成可靠的事实，再结合规则和 AI 帮用户理解风险。

**C++** 会在后续逐步承担 Windows 底层采集与系统能力层，学习并使用 Windows API、进程、权限、服务和 ETW 等能力。两者一起服务于同一个目标：从启动项、进程、日志、注册表、网络和权限等方向，构建并不断创新 AI Windows 安全助手。
